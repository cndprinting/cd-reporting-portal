...
export async function POST(req: NextRequest) {
  ...
  const { name, email, company, phone, message, hp } = body as Record<
    string,
    string | undefined
  >;
  ...
  try {
    await sendEmail({
      to: recipients,
      subject: `🎯 New MailerCity lead — ${name.trim()}${company ? ` (${company.trim()})` : ""}`,
...
<div style="font-size:11px;color:#6B6660;text-transform:uppercase;letter-spacing:0.1em;margin-top:4px;">Landing-page lead</div>
</td></tr>
